...
#inFile = 'PERMIAN_BASIN_298_Production_TEST_02.98f'
...
for line in fileinput.input(inFile):
    ...
    if match:
        ...
        if match:
            if not first_E:
                tstWell_toFileStack += ','.join(tstWell_toFile).replace(' ', '') + '\r\n'
            tstWell_toFile = tstWell[:]
        ...
        if match:
            ...
            outFile_tst.write(tstWell_toFileStack)
            tstWell_toFileStack = list()
